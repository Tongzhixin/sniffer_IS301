...
class Core():
    ...
    def process_packet(self, packet, writer):
        ...
        try:
            ...
            if not self.pause_flag and packet.name == "Ethernet":
                ...
                item = QTreeWidgetItem(self.main_window.infoTree)
                # 根据协议类型不同设置颜色
    ...
    def on_click_item(self, this_id):
        ...
        try:
            ...
            first_layer.append("[Time delta from previous captured frame: %f seconds]" %delta_time)
    ...
    def on_rightclick_item(self, idList):
        """
        处理点击列表中的多项 :parma idList: 处理选中的包
        """
        plist = []
        tmp = None
        id_dict = {}
    ...
    def get_next_layer(self, packet):
        ...
        try:
            ...
            if protocol == "NoPayload":
                ...
                if ether_type == "IP":
                    ether_type += "v4"
                ether_proto = ("Type: %s (%s)" %(ether_type, hex(packet[packet_class].type)))
            ...
            # IPv4
                ...
                network = "Internet Protocol Version 4, Src: %s, Dst: %s" % (ip_src, ip_dst)
                first_return.append(network)
                next_layer.append("Version: %d" % packet[packet_class].version)
                next_layer.append("Header Length: %d bytes (%d)" %(packet[packet_class].ihl << 2, packet[packet_class].ihl))
                next_layer.append("Differentiated Services Field: %s" % hex(packet[packet_class].tos))
                next_layer.append("Total Length: %d" % packet[packet_class].len)
                next_layer.append("Identification: %s (%d)" % (hex(packet[packet_class].id), packet[packet_class].id))
                next_layer.append("Flags: %d (%s)" % (packet[packet_class].flags,hex(packet[packet_class].flags.value)))
                next_layer.append("Fragment offset: %d" % packet[packet_class].frag)
                next_layer.append("Time to live: %d" % packet[packet_class].ttl)
                next_protocol = packet.payload.name
                if next_protocol == "IP":
                    next_protocol += "v4"
                next_layer.append("Protocol: %s (%d)" %(next_protocol, packet[packet_class].proto))
                ...
                next_layer.append("[Header checksum status: " + "Correct]" if ip_check == ip_chksum else "Incorrect]")
            ...
            # IPv6
                ...
                network = ("Internet Protocol Version 6, Src: %s, Dst: %s" %(ipv6_src, ipv6_dst))
                first_return.append(network)
                next_layer.append("Version: %d" % packet[packet_class].version)
                next_layer.append("Traffice Class: %s" % hex(packet[packet_class].tc))
                next_layer.append("Flow Label: %s" % hex(packet[packet_class].fl))
                next_layer.append("Payload Length: %d" % packet[packet_class].plen)
                next_protocol = packet.payload.name
                if next_protocol == "IP":
                    next_protocol += "v4"
                next_layer.append("Next Header: %s (%d)" %(next_protocol, packet[packet_class].nh))
                ...
                first_return.append(network)
                next_layer.append("Hardware type: %d" % packet[packet_class].hwtype)
                ...
                next_layer.append(temp_str)
                next_layer.append("Sender MAC address: %s" % packet[packet_class].hwsrc)
                next_layer.append("Sender IP address: %s" % packet[packet_class].psrc)
                next_layer.append("Target MAC address: %s" % packet[packet_class].hwdst)
                next_layer.append("Target IP address: %s" % packet[packet_class].pdst)
            # 第四层: 传输层
            elif protocol == "TCP" or protocol == "TCP in ICMP":
                src_port = packet[packet_class].sport
                dst_port = packet[packet_class].dport
                transport = ("Transmission Control Protocol, Src Port: %d, Dst Port: %d" % (src_port, dst_port))
                ...
                next_layer.append("Acknowledgment number: %d" % packet[packet_class].ack)
                tcp_head_length = packet[packet_class].dataofs
                next_layer.append("Header Length: %d bytes (%d)" %(tcp_head_length << 2, tcp_head_length))
                next_layer.append("Flags: %s (%d)" % (hex(packet[packet_class].flags.value), packet[packet_class].flags))
                next_layer.append("Window size value: %d" % packet[packet_class].window)
                ...
                next_layer.append("[Checksum status: " + "Correct]"if tcp_check == tcp_chksum else "Incorrect]")
                next_layer.append("Urgent pointer: %d" % packet[packet_class].urgptr)
                ...
                transport = ("User Datagram Protocol, Src Port: %d, Dst Port: %d" %(src_port, dst_port))
                first_return.append(transport)
                next_layer.append("Source Port: %d" % src_port)
                next_layer.append("Destination Port: %d" % dst_port)
                ...
                next_layer.append("[Checksum status: " + "Correct]" if udp_check == udp_chksum else "Incorrect]")
                ...
                if length > 0:
                    ...
                    if src_port == 1900 or dst_port == 1900:
                        first_return.append("Simple Service Discovery Protocol")
                ...
                if packet_type == 0 or packet_type == 8 or protocol == "ICMP in ICMP":
                    next_layer.append("Identifier: %d (%s)" % (packet[packet_class].id, hex(packet[packet_class].id)))
                    next_layer.append("Sequence number: %d (%s)" % (packet[packet_class].seq, hex(packet[packet_class].seq)))
                    data_length = len(packet.payload)
                    if data_length > 0:
                        next_layer.append("Data (%d bytes): %s" %(data_length, packet[packet_class].load.hex()))
                ...
                if protocol.lower().find("option") == -1:
                    ...
                    if proto_type in icmpv6_code:
                        if packet_code in icmpv6_code[proto_type]:
                            temp_str += " (%s)" % icmpv6_code[proto_type][packet_code]
                    next_layer.append(temp_str)
                    ...
                    next_layer.append("[Checksum status: " + "Correct]" if icmpv6_check == icmpv6_cksum else "Incorrect]")
                    if proto_type == "Echo Request" or proto_type == "Echo Reply":
                        ...
                        next_layer.append("Target Address: %s" % packet[packet_class].tgt)
                        ...
                        next_layer.append("Length: %d (%s)" %(packet[packet_class].length, hex(packet[packet_class].length)))
                        next_layer.append("Unused: %d" % packet[packet_class].unused)
                    ...
                    if proto_type == 1 or proto_type == 2:
                        ...
                        length = packet[packet_class].len
                        next_layer.append("Length: %d (%d bytes)" % (length, length << 3))
                    ...
                    # Prefix Information
                        ...
                        length = packet[packet_class].len
                        next_layer.append("Length: %d (%d bytes)" % (length, length << 3))
                        next_layer.append("Prefix Length: %d" %packet[packet_class].prefixlen)
                        set_str = "Set (1)"
                        not_set_str = "Not set (0)"
                        next_layer.append("On-link flag (L): %s" %set_str if packet[packet_class].L == 1 else not_set_str)
                        next_layer.append("Autonomous address-configuration flag (A): %s" % set_str if packet[packet_class].A == 1 else not_set_str)
                        next_layer.append("Router address flag(R): %s" % set_str if packet[packet_class].R == 1 else not_set_str)
                        next_layer.append("Valid Lifetime: %d" % packet[packet_class].validlifetime)
                        next_layer.append("Preferred Lifetime: %d" %packet[packet_class].preferredlifetime)
                        next_layer.append("Reserverd: %d" % packet[packet_class].res2)
                    ...
                    # MTU
                        ...
                        length = packet[packet_class].len
                        next_layer.append("Length: %d (%d bytes)" % (length, length << 3))
                        next_layer.append("Reserverd: %d" % packet[packet_class].res)
            ...
            # TLS
                ...
                while len(https) >= 10:
                    if https[2:4] == '03' and https[
                        ...
                        next_layer.append("Content Type: %s (%d)" % (cont_type, int(https[0:2], 16)))
                        next_layer.append("Version: %s (0x%s)" % (protocol, https[2:6]))
    ...
    def start_capture(self, netcard=None, filters=None):
        ...
        if self.stop_flag:
            ...
            self.packet_id = 1
            self.clean_out()
            temp = NamedTemporaryFile(suffix=".pcap", prefix=str(int(time.time())), delete=False)
            self.temp_file = temp.name
            temp.close()
        # 如果从暂停开始
    ...
    def open_pcap_file(self):
        ...
        if filename == "":
            return
        self.main_window.infoTree.clear()
        self.main_window.infoPacket.clear()
        ...
        if filename.find(".pcap") == -1:
            ...
            filename = filename + ".pcap"
        self.packet_id = 1
        self.main_window.infoTree.setUpdatesEnabled(False)
        shutil.copy(filename, self.temp_file)
        sniff(
            prn=(lambda x: self.process_packet(x, None)),
            store=False,
            offline=self.temp_file)
        self.main_window.infoTree.setUpdatesEnabled(True)
        self.stop_flag = True
        self.save_flag = True
